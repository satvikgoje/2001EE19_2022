...
def octant_longest_subsequence_count_with_range():
    ...
    arr = ["+1", "-1", "+2", "-2", "+3", "-3", "+4", "-4"]
    for i in range(8):
        df1.loc[i, "Octant "] = arr[i]  # appending values in octant column
    ...
    max_count1 = [0]*8  # initlizing a max_count1 with all zeroes
    d1 = {"+1": 0, "-1": 1, "+2": 2, "-2": 3,"+3": 4, "-3": 5, "+4": 6, "-4": 7}  # creating a dictionary
    ...
    t=0
    for i in range(8):
        df1.loc[t," Octant "]=arr[i]
        df1.loc[t," Longest Subsequence Length"]=max_count[i]
        df1.loc[t," Count"]=max_count1[i]
        t+=1
        df1.loc[t," Octant "]="Time"
        df1.loc[t," Longest Subsequence Length"]="From"
        df1.loc[t," Count"]="To"
        t+=1
        for j in range(max_count1[i]):
            df1.loc[t," Longest Subsequence Length"]=0.01*(time_range[d1[arr[i]]][j])-0.01*(max_count[i]-1)
            df1.loc[t," Count"]=0.01*time_range[d1[arr[i]]][j]
            t+=1


    df1.to_excel('octant_output1.xlsx', index=False)
